...
# ---------------------------------------------------------------------------
# OLLAMA ROUTER — loopback-only proxy with pi-gate admission
# ---------------------------------------------------------------------------


class OllamaRouter:
    """Route text/chat requests to a local Ollama instance — loopback only.

    Zero-telemetry by construction: the upstream URL is validated against a
    strict allowlist (localhost / 127.0.0.1 / [::1]) at construction, so this
    router is structurally incapable of reaching a remote host. Every request
    passes the PiEnginePacer admission gate before touching the wire, and
    every response is sealed with the node's HMAC brand.
    """
    ...
    def __init__(
        ...
        self.daemon = daemon
        self.upstream = self._validate_upstream(upstream)
        self.timeout_s = float(timeout_s)

    def _validate_upstream(self, raw: str) -> str:
        """Reject any upstream that is not a loopback address."""
        from urllib.parse import urlparse

        host = (urlparse(raw).hostname or "").lower()
        if host not in self.ALLOWED_HOSTS:
            raise ValueError(
                f"Ollama upstream '{raw}' rejected: only localhost / 127.0.0.1 / [::1] "
                "are permitted on this air-gapped node."
            )
        return raw.rstrip("/")
    ...
    def chat(
        ...
        **options: Any,
        ...
        """Forward a chat request to /api/chat through the pi admission gate."""
        payload = json.dumps({
            "model": model,
            "messages": messages,
            "stream": stream,
        ...
        return self._forward("/api/chat", payload)

    def generate(self, model: str, prompt: str, stream: bool = False, **options: Any) -> Dict[str, Any]:
        """Forward a completion request to /api/generate through the pi gate."""
        payload = json.dumps({
            "model": model,
            "prompt": prompt,
            "stream": stream,
        ...
        return self._forward("/api/generate", payload)

    # -- internal plumbing -------------------------------------------------------

    def _forward(self, path: str, payload: bytes) -> Dict[str, Any]:
        """Pi-gate admission, then loopback POST, then brand-sealed response."""
        weight = float(len(payload))
        verdict = self.daemon.pacer.admit(raw_weight=weight, queued=0.0, sustainable=1.0)
        if not verdict.admitted:
            return self.daemon.brand.seal({
                "kind": "ollama_response",
                "error": f"deferred by pi gate: {verdict.reason}",
        ...
        try:
            ...
            return self.daemon.brand.seal({
                "kind": "ollama_response",
                "error": f"HTTP {exc.code} from Ollama: {detail}",
                "hint": f"If the model is missing, run: ollama pull <model>",
            })
        except (urllib.error.URLError, ConnectionError, OSError) as exc:
            return self.daemon.brand.seal({
                "kind": "ollama_response",
                "error": f"Ollama unreachable at {self.upstream}: {exc}",
                "hint": "Start the daemon with: ollama serve",
            })
    ...
    def serve(self, host: str = "127.0.0.1", port: int = 11834) -> None:
        """Run a loopback HTTP proxy exposing /chat and /generate routes.

        Any local tool (VS Code extension, curl, the web console) can POST
        {"model": ..., "messages": [...]} to http://127.0.0.1:11834/chat and
        receive a brand-sealed Ollama response — still 100% loopback.
        """
        if host not in self.ALLOWED_HOSTS:
            raise ValueError(f"serve() host '{host}' rejected: loopback only.")
        router = self
        ...
        class Handler(BaseHTTPRequestHandler):
            def do_POST(self):  # noqa: N802 - BaseHTTPRequestHandler API
                ...
                if self.path == "/chat":
                    result = router.chat(
                        body.get("model", "qwen2.5-coder:latest"),
                        body.get("messages", []),
                    )
                    ...
                    result = router.daemon.heartbeat()
                else:
            ...
            def do_GET(self):  # noqa: N802
                if self.path in ("/", "/health"):
                    self._send(200, router.daemon.heartbeat())
                else:
                    self._send(404, {"error": f"unknown route: {self.path}"})
        ...
        print(f"[ggg-daemon] Ollama router listening on http://{host}:{port} "
...
def _self_test() -> None:
    ...
    # 4. Ollama router: loopback enforcement + unreachable-upstream handling.
    router = OllamaRouter(daemon)
    assert router.upstream == "http://localhost:11434"
    try:
        OllamaRouter(daemon, upstream="http://192.0.2.10:11434")
        raise AssertionError("non-loopback upstream must be rejected")
    except ValueError:
        pass  # correctly rejected
    resp = router.chat("qwen2.5-coder:latest", [{"role": "user", "content": "ping"}])
    assert daemon.brand.verify(resp), "router response must be brand-sealed"

    print(json.dumps(env, indent=2))
    print("[SELF-TEST OK] pi-engine gate, non-zero-sum balancer, HMAC brand, "
          "Ollama loopback router all verified.")


if __name__ == "__main__":
    if "--serve" in os.sys.argv:
        _d = GGGDaemon()
        OllamaRouter(_d).serve()
    else:
        _self_test()
