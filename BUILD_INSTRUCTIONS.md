...
Both the Tauri installers and the PyInstaller-built `ggg-daemon.exe` are
unsigned local binaries, so Windows SmartScreen will show a blue
**"Windows protected your PC"** dialog on first launch. This is a reputation
heuristic, not a malware detection — the binary simply has no commercial
code-signing certificate yet.

**Step by step:**

1. Click **"More info"** on the SmartScreen dialog (the "Don't run" button
   hides the option — you must expand first).
2. Confirm the publisher/app name matches the file you built or downloaded,
   then click **"Run anyway"**.
3. Windows remembers the choice for that exact file; future launches open
   normally.

**Verify before you trust.** Releases ship a `SHA256SUMS.txt` manifest —
confirm the binary hash matches before clicking through SmartScreen:
...
A hash match proves the binary is byte-identical to what the build pipeline
produced. No match = do not run.
